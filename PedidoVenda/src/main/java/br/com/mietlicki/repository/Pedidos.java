 ...
 * @author Rodrigo
 *
 *         Classe Dao responsável pelos metodos de
 *         inclução/alteração/listagem/remoção de pedidos
 *
 */
public class Pedidos implements Serializable {
	...
	public List<Pedido> buscarPedidosPorId(Long id) {
		...
		try {
			...
			Query query = this.manager
					.createQuery("from Pedido p where p.cliente.id =:id");
	...
	public List<ItemPedido> buscarItensPedido(Long id) {
		...
		try {
			...
			this.manager.getTransaction().begin();
			Query query = this.manager
					.createQuery("from ItemPedido ip where ip.id =:id");
			query.setParameter("id", id);
